  ...
  return (
    <div className="absolute inset-0 w-full h-full pointer-events-none z-0">
      ...
      <Canvas shadows dpr={[1.5, 2]} gl={{ antialias: true, toneMapping: THREE.ACESFilmicToneMapping }} camera={{ position: [0, 1.2, 8.5], fov: 45 }}>
        ...
        <Avatar gender={player1Avatar || 'male'} position={[-1.0, -0.4, 0]} isPlayer1={true} rotationY={0.3} />
        {player2Avatar ? (
            <Avatar gender={player2Avatar} position={[1.0, -0.4, 0]} isPlayer1={false} rotationY={-0.3} />
        ) : (
        ...
        <ContactShadows position={[0, -0.4, 0]} opacity={0.7} scale={20} blur={2} far={4} color="#000000" />
      </Canvas>
    </div>
  );
}
